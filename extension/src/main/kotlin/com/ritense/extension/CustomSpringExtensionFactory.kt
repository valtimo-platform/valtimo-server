...
class CustomSpringExtensionFactory(
    ...
    override fun <T : Any?> createWithSpring(extensionClass: Class<T>, applicationContext: ApplicationContext): T {
        return if (extensionClass.annotations.any { it.annotationClass == RepositoryAnnotation::class }) {
            factory.getRepository(extensionClass)
        } else {
            super.createWithSpring(extensionClass, applicationContext)
        }
    }
}