...
@Embeddable
public class OperatonProcessDefinitionKey implements ProcessDefinitionKey, Serializable {

    @Column(name = "camunda_process_definition_key", columnDefinition = "VARCHAR(255)")
    private String key;

    public OperatonProcessDefinitionKey(final String key) {
        assertArgumentNotNull(key, "key is required");
        assertArgumentLength(key, 255, "key max length is 255");
        this.key = key;
    }

    OperatonProcessDefinitionKey() {
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof OperatonProcessDefinitionKey that)) {
            return false;
        }
