...
internal class TypedObjectenApiClientTest {
    ...
    fun `should send get single object request and parse response`() {
        val client = TypedObjectenApiClient(restClientBuilder, outboxService, objectMapper)
    ...
    fun `should send outbox message on creating object`() {
        ...
        val result = client.createObject(
            TestAuthentication(),
            URI(objectUrl),
            TypedObjectRequest<JsonNode>(
                URI(objectTypesApiUrl),
                TypedObjectRecord<JsonNode>(
                    index = 1,
                    typeVersion = 2,
